<template>
  <div class="jokes-page">
    <div class="jokes-page-buttons">
          <select class="form-select form-select-lg mb-3" aria-label=".form-select-lg example" v-model="selectedCategory" @change="fetchSelectedCategoryJoke">
            <option value="" selected>Choose category</option>
            <option v-for="category in categories" :value="category" :key="category">{{ category }}</option>
          </select>
      <button-component class="another-joke-btn" :button-text="anotherJokeBtn" @click="getAnotherJoke"></button-component>
    </div>

    <div class="joke-container">
      <div v-if="joke.jokeText && !selectedCategory">
        <p>{{ joke.jokeText }}</p>
      </div>
      <div v-else-if="selectedCategory && selectedCategoryJoke">
        <p>{{ selectedCategoryJoke }}</p>
      </div>
      <div v-else>
        <p>Loading joke...</p>
      </div>
        <div v-if='joke.isAddedToFav === true' @click="addToFavourites()" class="fav-btn">
          <svg xmlns="http://www.w3.org/2000/svg" height="2em" viewBox="0 0 512 512">
            <path d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z"/>
          </svg>
        </div>
        <div v-else-if='joke.isAddedToFav === false' @click="addToFavourites()" class="fav-btn">
          <svg xmlns="http://www.w3.org/2000/svg" height="2em" viewBox="0 0 512 512">
            <path d="M225.8 468.2l-2.5-2.3L48.1 303.2C17.4 274.7 0 234.7 0 192.8v-3.3c0-70.4 50-130.8 119.2-144C158.6 37.9 198.9 47 231 69.6c9 6.4 17.4 13.8 25 22.3c4.2-4.8 8.7-9.2 13.5-13.3c3.7-3.2 7.5-6.2 11.5-9c0 0 0 0 0 0C313.1 47 353.4 37.9 392.8 45.4C462 58.6 512 119.1 512 189.5v3.3c0 41.9-17.4 81.9-48.1 110.4L288.7 465.9l-2.5 2.3c-8.2 7.6-19 11.9-30.2 11.9s-22-4.2-30.2-11.9zM239.1 145c-.4-.3-.7-.7-1-1.1l-17.8-20c0 0-.1-.1-.1-.1c0 0 0 0 0 0c-23.1-25.9-58-37.7-92-31.2C81.6 101.5 48 142.1 48 189.5v3.3c0 28.5 11.9 55.8 32.8 75.2L256 430.7 431.2 268c20.9-19.4 32.8-46.7 32.8-75.2v-3.3c0-47.3-33.6-88-80.1-96.9c-34-6.5-69 5.4-92 31.2c0 0 0 0-.1 .1s0 0-.1 .1l-17.8 20c-.3 .4-.7 .7-1 1.1c-4.5 4.5-10.6 7-16.9 7s-12.4-2.5-16.9-7z"/>
          </svg>
        </div>
    </div>

  </div>
</template>

<script>
import ButtonComponent from "@/components/ButtonComponent.vue";
import fetchFunction from "@/utilities";
import { CHUCK_API_URL } from "@/constants";

export default {
  components: {
    ButtonComponent,
  },
  data() {
    return {
      categoriesBtn: 'Choose category',
      anotherJokeBtn: 'Get another one',
      favouritesBtn: 'Add to favourites',
      joke: {
        id: '',
        jokeText: '',
        isAddedToFav: false
      },
      categories: [],
      selectedCategory: '',
      selectedCategoryJoke: '',
    };
  },
  created() {
    this.fetchRandomJoke();
    this.fetchAllCategories();
  },
  methods: {
    addToFavourites(){
      if(!this.joke.isAddedToFav){
        this.$store.commit('addToFavourites', this.joke);
      }else{
        this.removeFromFavourites();
      }
      this.joke.isAddedToFav = !this.joke.isAddedToFav;
    },
    removeFromFavourites() {
      const favouriteJokes = JSON.parse(localStorage.getItem('Favourites')) || [];
      const updatedFavourites = favouriteJokes.filter(item => item.id !== this.joke.id);
      localStorage.setItem('Favourites', JSON.stringify(updatedFavourites));
    },
    updateHeartIcon() {
      const favouriteJokes = JSON.parse(localStorage.getItem('Favourites')) || [];
      const targetJoke = favouriteJokes.find((joke) => joke.id === this.joke.id);
      if (targetJoke) {
        this.joke.isAddedToFav = true;
      } else {
        this.joke.isAddedToFav = false;
      }
    },
    async getAnotherJoke() {
      if (this.selectedCategory) {
        await this.fetchSelectedCategoryJoke();
      } else {
        await this.fetchRandomJoke();
      }
      this.updateHeartIcon();
    },
    async fetchRandomJoke() {
      const tempJoke = await fetchFunction(`${CHUCK_API_URL}/random`);
      this.joke.id = tempJoke.id;
      this.joke.jokeText = tempJoke.value;
    },
    async fetchAllCategories() {
      this.categories = await fetchFunction(`${CHUCK_API_URL}/categories`);
    },
    async fetchSelectedCategoryJoke() {
      if(this.selectedCategory){
        const categorizedJoke = await fetchFunction(`${CHUCK_API_URL}/random?category=${this.selectedCategory}`);
        this.selectedCategoryJoke = categorizedJoke.value;
        this.joke.jokeText = this.selectedCategoryJoke;
        this.joke.id = categorizedJoke.id;
      }else{
        this.selectedCategoryJoke = this.joke.jokeText;
      }
        this.updateHeartIcon();
    }
  }
};
</script>

<style>
.jokes-page {
  display: flex;
  flex-direction: column;
  align-items: center;
  background-color: #723b3b;
  min-height: 89.7%;
  position: relative;
}
.jokes-page-buttons {
  display: flex;
  width: 70%;
  justify-content: center;
  position: absolute;
  top: 20%;
}
.form-select {
  padding: 0 0 0 1rem !important;
  --bs-body-bg: #efd6d6 !important;
  height: 60px;
  width: 20% !important;
  font-size: 20px !important;
  border: none !important;
}
.another-joke-btn{
  margin-bottom: 1rem;
  width: 20%;
}
.fav-btn{
  color: #efefef;
  position: absolute;
  right: 80px;
  bottom: 30px;
}
.joke-container {
  width: 80%;
  height: 15rem;
  box-shadow: 5px 5px 5px #573131;
  border-radius: 20px;
  display: flex;
  align-items: center;
  justify-content: center;
  background-color: #e3d1d1;
  margin-top: 40px;
  position: absolute;
  top: 25%;
  font-size: 20px;
}
.joke-container>div {
  padding: 0 10px;
}
</style>
